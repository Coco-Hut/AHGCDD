...
class HyperDataset(object):
    ...
    def load_multi_data(self):
        ...
        for i, hg in enumerate(x_list):
            hypergraph = Data(
                x=torch.FloatTensor(hg["v_ft"]),
                hyperedge_index=hg["edge_index"],
        ...
        self.multi_hypergraphs = MultiHyperaphDatasets(
            x=hgs,
    ...
    def load_data(self):
        ...
        if dname in self.trad_list:
            ...
            if (f_noise is not None) and dname in [
                "walmart-trips-100",
                "house-committees-100",
                "walmart-trips",
                "house-committees",
    ...
    def calculate_memory_size(self):
        ...
        print(f"Total size of hypergraph data: {total_size_bytes} bytes")
        print(f"Total size of hypergraph data: {total_size_gb:.2f} GB")
    ...
    def generate_random_split(self, train_ratio: float = 0.1, val_ratio: float = 0.1, seed=None):
        ...
        train_set, val_set, test_set = random_split(
            torch.arange(0, self.num_nodes), (num_train, num_val, num_test), generator=generator
        )
