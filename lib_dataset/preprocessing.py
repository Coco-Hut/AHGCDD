...
def data_processing(args, db):
    ...
    if args.method in ["AllSetTransformer", "AllDeepSets"]:
        ...
        if args.add_self_loop:
            if args.add_self_loop_fast:
                data = Add_Self_Loops_fast(data)
            else:
...
def expand_edge_index(data, edge_th=0):
    ...
    for he_idx in range(num_nodes, num_edges + num_nodes):
        ...
        new_edge_ids = torch.LongTensor(np.arange(cur_he_id, cur_he_id + size_of_he)).repeat(size_of_he)
        ...
        for col_idx in range(new_n2he.shape[1]):
            tmp_node_id, tmp_edge_id = new_n2he[0, col_idx].item(), new_n2he[1, col_idx].item()
...
def Add_Self_Loops(data):
    ...
    new_edges = torch.zeros((2, num_nodes - len(set(skip_node_lst))), dtype=edge_index.dtype)
    tmp_count = 0
...
def Add_Self_Loops_fast(data):
    edge_index = data.edge_index
    num_nodes = data.n_x
    num_hyperedges = data.num_hyperedges

    if not ((num_nodes + num_hyperedges - 1) == edge_index[1].max().item()):
        print("num_hyperedges does not match! 2")
        return
    ...
    max_eid = int(edge_index[1].max().item())
    counts = torch.bincount(edge_index[1], minlength=max_eid + 1)

    singleton_eids = torch.nonzero(counts == 1, as_tuple=False).view(-1)

    if singleton_eids.numel() > 0:
        pos = torch.isin(edge_index[1], singleton_eids)
        skip_nodes = torch.unique(edge_index[0][pos])
    else:
        skip_nodes = edge_index.new_empty((0,), dtype=edge_index[0].dtype)

    add_mask = torch.ones(num_nodes, device=device, dtype=torch.bool)
    if skip_nodes.numel() > 0:
        add_mask[skip_nodes.long()] = False
    nodes_to_add = torch.nonzero(add_mask, as_tuple=False).view(-1)

    new_edge_idx_start = edge_index[1].max() + 1
    new_eids = new_edge_idx_start + torch.arange(nodes_to_add.numel(), device=device, dtype=edge_index[1].dtype)

    new_edges = torch.stack([nodes_to_add.to(dtype), new_eids], dim=0)

    edge_index2 = torch.cat([edge_index, new_edges], dim=1)
    sorted_idx = edge_index2[0].argsort()
    data.edge_index = edge_index2[:, sorted_idx].long()

    data.totedges = num_hyperedges + nodes_to_add.numel()
    return data
...
def rand_train_test_idx(label, train_prop=0.5, valid_prop=0.25, ignore_negative=True, balance=False):
    if not balance:
        if ignore_negative:
