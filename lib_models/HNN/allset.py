...
class HalfNLHconv(MessagePassing):
    ...
    def aggregate(self, inputs, index,
        ...
        aggr = aggr if aggr is not None else "add" # 默认赋予add，因为AllDeepSets场景下为add
        if aggr is None:
            raise ValueError("aggr was not passed!")
        return scatter(inputs, index, dim=self.node_dim, reduce=aggr)
    ...
    def forward(self, data):
        ...
        if self.GPR:
            ...
            for i, _ in enumerate(self.V2EConvs):
                e = F.relu(self.V2EConvs[i](x, edge_index, norm, self.aggr)) # 后续edge_dependent node classification task优化edge embedding
