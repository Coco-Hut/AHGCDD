...
def create_coo_from_edge_index(edge_index,is_star=False):
    ...
    if is_star:
        H = sp.coo_matrix((values, (row, col)), shape=(num_hyperedges, num_hyperedges)).astype(np.float32) # 这里用float32减小内存消耗
    else:
        H = sp.coo_matrix((values, (row, col)), shape=(num_nodes, num_hyperedges)).astype(np.float32) # 这里用float32减小内存消耗
